...
if (!class_exists("ASP_Helpers")) {
    ...
    class ASP_Helpers {
        ...
        public static function getCFValue($field, $r, $use_acf = false, $args = array(), $field_args = array()) {
            ...
            if( ( in_array($field, $datetime_fields) || isset($field_args['date_format']) ) &&
                isset($r->post_type) &&
                ...
                if ( $use_acf && function_exists('get_field') ) {
                    global $post;
                    $post = get_post($r->id);            
                    $mykey_values = get_field($field);
                    //var_dump($mykey_values);   
                    update_field($field, $mykey_values, $r->id);     
                    $mykey_values = get_field($field);
                    //var_dump($mykey_values);   
                    if ( !is_null($mykey_values) && $mykey_values != '' && $mykey_values !== false ) {
                        if ( is_array($mykey_values) ) {
        ...
        private static function toQueryArgs_Custom_Fields($sd, $o, $source = 'postmeta') {
            ...
            if ( isset( $o['aspf'] ) && !empty( $_cf_items ) ) {
                ...
                foreach ( $_cf_items as $u_data ) {
                    ...
                    if ( !isset($o['aspf'][ $data->asp_f_field . "_" . $cf_i ]) ) {
                        // Try correcting the field name, containing [] brackets etc..
                        $data->asp_f_field = str_replace(
                            array('[', ']'),
                    ...
                    // Reverse any correction, containing [] brackets etc..
                    $data->asp_f_field = str_replace(
                        array('!_brktl_!', '!_brktr_!'),
                        array('[', ']'),
                        $data->asp_f_field
